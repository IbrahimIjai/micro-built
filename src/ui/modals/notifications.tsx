...
export default function Notifications() {
  ...
  const filterNotifications = (tab: string) => {
    switch (tab) {
      ...
      case "repayments":
        return notifications.filter((n) => n.type === "repayment" || n.type === "overdue_repayment");
      //   case "inventory":
  ...
  const groupNotificationsByDate = (notifications: Notification[]) => {
    const grouped = notifications.reduce((acc, notification) => {
      const date = notification.date.toUpperCase();
      if (!acc[date]) {
        acc[date] = [];
      }
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button variant="secondary" className="rounded-full relative h-8 w-8" disabled>
      ...
      <PopoverContent className="w-96 p-0" align="end">
        ...
        <div className="p-4 text-sm text-muted-foreground border-b">
          All new notifications on loan requests, inventories and repayments will appear here.
        </div>
          ...
          <TabsContent value={activeTab} className="mt-0">
            <div className="max-h-96 overflow-y-auto">
              {Object.entries(groupedNotifications).map(([date, dateNotifications]) => (
                <div key={date}>
                  <div className="px-4 py-2 text-xs font-medium text-muted-foreground bg-gray-50">{date}</div>
