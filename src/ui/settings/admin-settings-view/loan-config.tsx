  ...
  return (
    <div className="p-3 lg:p-5 flex flex-col gap-8">
      <div className="flex flex-col gap-3">
        <Label htmlFor="interest-rate" className="text-muted-foreground font-normal text-sm">
          Interest Rate
        </Label>
...
function EditConfig({ rateKey, value }: Omit<UpdateRateDto & { rateKey: UpdateRateDto["key"] }, "key">) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Update {rateKey === "INTEREST_RATE" ? "Interest Rate" : "Management Fee Rate"}</DialogTitle>
        </DialogHeader>
        <Separator className="bg-[#F0F0F0]" />
        <div className="grid gap-4 p-4 sm:p-5">
          <Input type="number" value={newValue} onChange={(e) => setNewValue(parseFloat(e.target.value))} />
