...
const columns: ColumnDef<CustomerListItemDto>[] = [
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-3">
        <UserAvatarComponent id={row.original.id} name={row.original.name} className="w-8 h-8" />
  ...
  },
  {
    ...
    header: "Customer ID",
    cell: ({ row }) => {
      return <div className="font-medium text-green-600">{row.getValue("id")}</div>;
    },
  },
  {
    accessorKey: "",
    header: "Contact Info",
    cell: ({ row }) => <div className="text-muted-foreground">{row.original.contact ?? row.original.email}</div>,
  },
  {
